 ...
 *
 * $Id: index.php,v 1.59 2007/06/03 18:21:36 jon Exp $
 **/
...
// Special admin access session var
$admin_access = false;

if (isset($_SESSION['adminaccess']) && $_SESSION['adminaccess'] > time()) {
	$admin_access = true;
} else if (isset($admin->post['password'])) {
	if (md5($admin->post['password']) == $admin->user['user_password']) {
		$admin_access = true;
	}
}

if ($admin_access) {
	$_SESSION['adminaccess'] = time() + ADMIN_TIMEOUT;
}else{
	echo "
	...
	<form action='{$admin->self}' method='post' autocomplete='off'>
	<table width='50%' align='center' bgcolor='#454545' cellpadding='0' cellspacing='0' border='0'><tr><td><table width='100%' cellpadding='5' cellspacing='1' border='0'>
		<tr>
			<td class='header' colspan='2'>Administrator Control Panel</td>
		</tr>
		<tr>
			<td class='labellight' style='width:50%'><b>Password</b></td>
  			<td class='tablelight'>
  	 		    <input class='input' type='password' name='password' size='40' />
  			</td>
 		</tr>
 		<tr>
	...
	die; // Don't allow further processing until a successful login!
}

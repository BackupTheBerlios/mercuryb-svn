 ...
 *
 * $Id: settings.php,v 1.45 2007/04/07 09:26:23 jon Exp $
 **/
	...
	function execute()
	{
		...
		case 'basic':
			...
			if (!extension_loaded('zlib')) {
   				/**
				 * Build a warning for the user that turning on GZip compression
			     * could cause the borad to be garbled or not work at all + disable option
			     **/
				$gzip = "<br/><br/><font color='red'><b>Warning! : </b>MercuryBoard has detected that your server does not support this feature and disabled it, turning it on could cause the board to malfunction</font>";
 				$enabled_gzip = "disabled='disabled'";			
			}
			
			if (!extension_loaded('gd')) {
				/**
				 * Build a warning for the user to say that turning on anti-robot
				 * registration will cause registration to fail if the correct GD
				 * libraries are not installed + disable option
				 **/
				$gd = "<span class='tiny'><br/><br/><font color='red'><b>Warning! : </b>MercuryBoard has detected that your server does not support this feature and has disabled it, turning it on will cause all registrations to fail</font></span>";
				$enabled_gd = "disabled='disabled'";
			}
			
			$get_terms = $this->db->fetch("SELECT settings_term FROM {$this->pre}settings");
			$terms_conditions = htmlspecialchars($get_terms['settings_term']);
			...
			$this->table
				...
				<tr>
					<td class='label" . $this->iterate() . "'>Anti-Robot Registration {$gd}</td>
					<td class='table" . $this->lastValue() . "'>
						<input type='radio' name='register_image' value='1'" . ($this->sets['register_image']  ? ' checked' : null) . " id='register_image1' {$enabled_gd}><label for='register_image1'>Enabled</label><br />
				...
				<tr>
					<td class='label" . $this->iterate() . "' valign='top'>Terms & Conditions Agreement<br /><span class='tiny'>Type your agreement text here to display on the registration screen, HTML can be used for formatting text and line spacing.<br /> <i>You must enable Terms And Conditions below in order to use this feature<i/></span></td>
					<td class='table" . $this->lastValue() . "'><textarea class='input' name='term_agree' rows='10' cols='40'>$terms_conditions</textarea></td>
				</tr>
				<tr>
					<td class='label" . $this->iterate() . "'>Terms & Conditions<br/><span class='tiny'>Allows you to set up an agreement that your users have to accept before registering</span></td>
					<td class='table" . $this->lastValue() . "'>
						<input type='radio' name='terms' value='1'" . ($this->sets['terms'] ? ' checked' : null) . " id='terms2'><label for='terms2'>Enabled</label><br />
						<input type='radio' name='terms' value='0'" . (!$this->sets['terms'] ? ' checked' : null) . " id='terms1'><label for='terms1'>Disabled</label>
					</td>
				</tr>
			$this->etable
			<br /><br />
			...
			$this->table
				...
				<tr>
					<td class='label" . $this->iterate() . "' width='50%'>GZIP Compression<br /><span class='tiny'>Improves speed. Disable if the board appears jumbled or blank.{$gzip}</span></td>
					<td class='table" . $this->lastValue() . "'>
						<input type='radio' name='output_buffer' value='1'" . ($this->sets['output_buffer']  ? ' checked' : null) . " id='output_buffer1' {$enabled_gzip}><label for='output_buffer1'>Enabled</label><br />
		...
		case 'update':
			...
			$terms_conditions = $this->post['term_agree'];
			$vartypes = array(
				'db_host' => 'string',
				'db_name' => 'string',
				'db_user' => 'string',
			...
			if (isset($this->get['db'])) {
				...
				$this->write_sets();
				$this->db->query("UPDATE {$this->pre}settings SET settings_term='{$terms_conditions}'");
			}

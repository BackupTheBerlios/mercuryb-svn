 ...
 *
 * $Id: perms.php,v 1.52 2007/07/10 17:55:56 jon Exp $
 **/
...
class perms extends admin
{
	function execute()
	{
		...
		$perms = array(
				...
				'view_invisible_users' => 'View hidden members using the board',
				'forum_view' => 'View the forum',
				'forum_subscribe' => 'Can subscribe to this forum and all its topics',
				'post_viewip' => 'View user IP addresses',
				'topic_view' => 'View topics',
				'topic_view_unpublished' => 'View Unpublished topics',
				'poll_create' => 'Create polls',
				'poll_vote' => 'Create votes',
				'post_create' => 'Create replies',
				'post_counting' => 'Posts contribute to users post count',
				...
				'topic_publish' => 'Publish or unpublish any topic',
				'topic_publish_auto' => 'New topics are marked as published',
				'topic_rate' => 'Rate topics in this forum',
		...
		if ($mode == 'user') {
			...
			{ 
					...
					if ($sub['subscription_type'] == 'forum') {
						...
						if (!$perms->auth('forum_subscribe', $sub['subscription_item'])) { //if user can no longer subscribe to a forum
							$this->db->query("DELETE FROM {$this->pre}subscriptions WHERE subscription_user={$sub['user_id']} AND subscription_item={$sub['subscription_item']}");	
						}
					}else{
						...
						if (!$perms->auth('forum_subscribe', $check['topic_forum'])) { //if user can no longer subscribe to a forum
							$this->db->query("DELETE FROM {$this->pre}subscriptions WHERE subscription_user={$sub['user_id']} AND subscription_item={$sub['subscription_item']}");	
						}
					}
			}

 ...
 *
 * $Id: user.php,v 1.8 2007/07/24 18:37:55 jon Exp $
 **/
...
class user
{
	...
	function login()
	{
		if(isset($this->cookie[$this->sets['cookie_prefix'] . 'user']) && isset($this->cookie[$this->sets['cookie_prefix'] . 'pass'])){
			...
			$user = $this->db->fetch("SELECT m.*, s.skin_dir, g.group_perms, g.group_name, t.membertitle_icon 
									  FROM {$this->pre}users m, {$this->pre}skins s, {$this->pre}groups g 
									  LEFT JOIN {$this->pre}membertitles t ON t.membertitle_id = m.user_level
									  WHERE m.user_id = '{$cookie_user}' AND 
									  	m.user_password = '{$cookie_pass}' AND 
			...
			$user = $this->db->fetch("SELECT m.*, s.skin_dir, g.group_perms, g.group_name, t.membertitle_icon 
									  FROM {$this->pre}users m, {$this->pre}skins s, {$this->pre}groups g 
									  LEFT JOIN {$this->pre}membertitles t ON t.membertitle_id = m.user_level 
									  WHERE m.user_id = '{$session_user}' AND 
									  	m.user_password = '{$session_pass}' AND 
									  	s.skin_dir = m.user_skin AND 
									  	g.group_id = m.user_group 
									  LIMIT 1");
			...
			$user = $this->db->fetch("SELECT m.*, s.skin_dir, g.group_perms, g.group_name 
									  FROM {$this->pre}users m, {$this->pre}skins s, {$this->pre}groups g 
									  WHERE m.user_id = " . USER_GUEST_UID . " AND 
									  	s.skin_dir = m.user_skin AND 
									  	g.group_id = m.user_group 
									  LIMIT 1");
		}

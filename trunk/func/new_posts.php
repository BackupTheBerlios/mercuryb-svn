<?php
  ...
  *
  * $Id: new_posts.php,v 1.5 2007/06/10 10:50:41 jon Exp $
  **/
 /**
 ...
 class new_posts extends mercuryboard
 {
 	function execute()
	{
		...
		$pagelinks  = $this->get_pages($topicCount, 'a=new_posts', $min, $n);
	...
	function check_new_posts($n, $min)
	{
		...
		SELECT DISTINCT(t.topic_id), 
			...
			t.topic_icon, t.topic_title, t.topic_rate,
		...
		LIMIT
			...
			{ 
				...
				if (!($new_post['topic_modes'] & TOPIC_PUBLISH)) {
					$new_post['topic_title'] = "<i>" . $new_post['topic_title'] . "</i>";
				}
				...
				if (($new_post['topic_rate'] != 0) && ($this->perms->auth('topic_rate', $new_post['topic_forum']))) {
					$topic_rate = '<img src="./skins/' . $this->skin . '/images/' . $new_post['topic_rate'] . '.gif" alt="' . $this->lang->new_posts_rating . '" align="right"/>';
				}else{
					$topic_rate = "";
				}

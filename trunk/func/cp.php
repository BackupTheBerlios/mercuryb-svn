 ...
 *
 * $Id: cp.php,v 1.47 2007/06/12 09:50:38 jon Exp $
 **/
...
class cp extends mercuryboard
{
	...
	function check_pass($passA, $passB, $old_pass)
	{
		...
		if (!preg_match('/[a-z0-9_\- ]{5,}$/i', $passA)) {
			return PASS_INVALID;
		}
	...
	function edit_profile()
	{
		if ( !$this->perms->auth('edit_profile')) {
			return $this->message($this->lang->cp_editing_profile, $this->lang->cp_editing_profile_disabled);
		}
	...
	function edit_avatar()
	{
		if ( !$this->perms->auth('edit_avatar')) {
			return $this->message( $this->lang->cp_editing_avatar, $this->lang->cp_editing_avatar_disabled);
		}
	...
	function add_sub()
	{
		...
		if ($this->get['type'] == 'forum') {
			if (!$this->perms->auth('forum_subscribe', $this->get['item'])) {
				return $this->message($this->lang->cp_cp, $this->lang->cp_sub_perm);	
			}
		} else {
			if (!isset($this->get['forum'])) {
				return $this->message($this->lang->cp_cp, $this->lang->cp_sub_no_params);
			}
			
			$this->get['forum'] = intval($this->get['forum']);
			
			if (!$this->perms->auth('forum_subscribe', $this->get['forum'])) {
				return $this->message($this->lang->cp_cp, $this->lang->cp_sub_perm);
			}
		}

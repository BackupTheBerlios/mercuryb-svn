 ...
 *
 * $Id: pm.php,v 1.50 2007/06/11 12:18:44 jon Exp $
 **/
...
class pm extends mercuryboard
{
	...
	function folder()
	{
		...
		{
			if (!$pm['pm_read']) {
				$icon = '<img src="./skins/' . $this->skin . '/images/newopen.gif" alt="' . $this->lang->pm_pm . '" />';
			} else {
				$icon = '<a href="' . $this->self . '?a=pm&amp;s=unread&amp;m=' . $pm['pm_id'] . '"><img src="./skins/' . $this->skin . '/images/open.gif" alt="' . $this->lang->pm_pm . '" /></a>';
			}
	...
	function mark_unread()
	{
		...
		$this->db->query('UPDATE ' . $this->pre . 'pmsystem SET pm_read=0 WHERE pm_id=' . $this->get['m'] . ' AND pm_to=' . $this->user['user_id']);
	...
	function view_buddies()
	{
        ...
        $Pages = $this->get_pages($buddyCount['c'], "a=pm&amp;s=buddies", $this->get['min'], $this->get['num']);
	...
	{	
		...
		//make sure the user is not adding themselves to the list
		if ($buddy == $this->user['user_id']) {
			return $this->message($this->lang->pm_personal_msging, $this->lang->pm_buddy_yourself, $this->lang->pm_continue, "{$this->self}?a=topic&amp;t={$this->get['t']}");
		}

 ...
 *
 * $Id: topic.php,v 1.83 2007/06/17 10:06:44 jon Exp $
 **/
...
class topic extends mercuryboard
{
	...
	function get_topic()
	{
		...
		$topic = $this->db->fetch('
		SELECT
		    t.topic_title, t.topic_description, t.topic_modes, t.topic_starter, t.topic_forum, t.topic_replies, t.topic_poll_options, t.topic_rate, f.forum_name
		FROM
		...
		if ($topic['topic_rate'] != 0) {
			$topic_rate = '<img src="./skins/' . $this->skin . '/images/' . $topic['topic_rate'] . '.gif" alt="' . $this->lang->topic_rating . '"/>';
		}else{
			$topic_rate = "";
		}
	 ...
	 * Topic Rating
	 *
	 * Calculates the percentage rating for a topic & what icon is needed as
	 * well as counting the rate just posted
	 * @author Jonathan West <jon@quicksilverforums.com>
	 * @since 1.1.6
	 **/
	function rate_topic()
	{
		if (!$this->perms->auth('topic_rate', $this->get['f'])) {
			return $this->message($this->lang->topic_error, $this->lang->topic_rate_perm);
		}
		
		$owner = $this->db->fetch("SELECT topic_starter FROM {$this->pre}topics WHERE topic_id='{$this->get['t']}'");

		if ($owner['topic_starter'] === $this->user['user_id']) { //topic owner cannot rate
			return $this->message($this->lang->topic_error, $this->lang->topic_rate_own);
		}
		
		if (!isset($this->post['rate'])) {
			return $this->message($this->lang->topic_error, $this->lang->topic_no_rate_selected);
		}
		
		$user_rated = $this->db->fetch("SELECT rate_user FROM {$this->pre}rates WHERE rate_user='" . $this->user['user_id'] . "' AND rate_topic='" . $this->get['t'] ."'");
		
		if (!$user_rated) { //if the user has not already rated this topic
			$current_rate = $this->db->query("SELECT rate_rated FROM {$this->pre}rates WHERE rate_topic='" . $this->get['t'] ."'");
			
			$raw_rate = 0;
			$number_rates = 1; // user that is rating the topic
			
			while ($rate = $this->db->nqfetch($current_rate))
			{
				$raw_rate .= $rate['rate_rated'];
				$number_rates++;
			}
			
			$topic_raw_rate = $raw_rate + $this->post['rate'];
			
			$topic_rate = ($topic_raw_rate / ($number_rates * 5)) * 100; // Work out the percentage rate of the topic

			$topic_rate_icon = 0; 
			switch($topic_rate) { // find what icon is needed
			case ($topic_rate <= 20):
				$topic_rate_icon = 1;
				break;
			
			case (($topic_rate > 20) && ($topic_rate <= 40)):
				$topic_rate_icon = 2;
				break;
			
			case (($topic_rate > 40) && ($topic_rate <= 60)):
				$topic_rate_icon = 3;
				break;
				
			case (($topic_rate > 60) && ($topic_rate <= 80)):
				$topic_rate_icon = 4;
				break;
				
			case (($topic_rate > 80) && ($topic_rate <= 100)):
				$topic_rate_icon = 5;
				break;
			}
			
			$this->db->query("INSERT INTO {$this->pre}rates (rate_user, rate_topic, rate_rated) VALUES ('" . $this->user['user_id'] . "', '" . $this->get['t'] . "',  '" . $this->post['rate'] . "')");
			$this->db->query("UPDATE {$this->pre}topics SET topic_rate='$topic_rate_icon' WHERE topic_id='{$this->get['t']}'");

			return $this->message($this->lang->topic_rating, $this->lang->topic_rated);
		}else{
			return $this->message($this->lang->topic_rating, $this->lang->topic_rated_already);
		}

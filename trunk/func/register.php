 ...
 *
 * $Id: register.php,v 1.34 2006/11/18 11:24:53 jon Exp $
 **/
...
class register extends mercuryboard
{
	...
	function registration()
	{
		if (!isset($this->post['submit'])) {
			...
			if ($this->sets['terms']) {
				// Keep alternating colour scheme
				$term_header = ($image ? 'tabledark' : 'tablelight');
				$term_content = ($image ? 'tablelight' : 'tabledark');
				
				$terms = $this->db->fetch("SELECT settings_term FROM {$this->pre}settings");
				$terms_conditions = $terms['settings_term'];
			}
			...
			if ($this->sets['terms']) {
				if (!isset($this->post['agree'])) { // Check if the user accepts terms & condtions
					return $this->message($this->lang->register_reging, $this->lang->register_condition_failed);
				}
			}

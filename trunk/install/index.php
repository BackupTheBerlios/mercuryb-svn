 ...
 *
 * $Id: index.php,v 1.42 2007/01/13 10:44:18 jon Exp $
 **/
	...
	case 'install':
		...
		default:
			...
			<tr>
				...
				<td><input class='input' type='password' name='db_pass' value='{$set['db_pass']}' /></td>
			</tr>
			<tr>
	...
	case 'upgrade':
		...
		default:
			...
			<tr>
				...
				<td><label for='upgrade15'>MercuryBoard v1.1.5</label></td>
			</tr>
			<tr>
			...
			if (!empty($add_permission)) {
				foreach ($add_permission as $id => $default)
				{
					...
					while ($mb->perms->get_group())
					{
						...
						if ($mb->perms->auth('is_admin')) {
							$perm_on = true;
						}
						if (!$mb->perms->auth('do_anything')) {
							$perm_on = false;
						}
						if ($mb->perms->is_guest) {
							$perm_on = false;
						}
						
						$mb->perms->add_perm($id, $perm_on);
						$mb->perms->update();
					}
			
					// Users
					while ($mb->perms->get_group(true))
					{
						$perm_on = $default;
